...
import { FaBookmark } from "react-icons/fa"; // ✅ Added for Save icon

// Define the required minimum score to unlock Intermediate lessons
const PREREQUISITE_SCORE = 5;

export default function JavaScriptInter() {
  const pageSize = 7;
  ...
  const loadUserScore = () => {
    try {
      ...
      if (storedMarksString) {
        ...
        const score = subjectMarks.JavaScript || 0;
        setJavaScore(score);
  ...
  // ✅ Fetch Intermediate Java lessons
  const fetchPaginatedLessons = async (page = 0) => {
    try {
      const response = await LessonService.getJavaScriptLessons(page, pageSize);
      if (response.status === 200) {
        ...
        const intermediateLessons = lessons.filter(
          (lesson) =>
            lesson.lessonLevel &&
            lesson.lessonLevel.toLowerCase() === "intermediate"
        );
  ...
  const handleLockedClick = (e) => {
    if (!isAuthorized) {
      e.preventDefault();
      toast.error(
        `You need a Java score of ${PREREQUISITE_SCORE} to access Intermediate lessons. Current score: ${javaScore}`
      );
    }
  };
  ...
  if (loading || statusLoading)
    return (
      <p className="text-center mt-10">
        Loading JavaScript Intermediate lessons...
      </p>
    );
