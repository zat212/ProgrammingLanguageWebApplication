...
export default function JavaScriptBeginner() {
  const pageSize = 7;
  const [totalPages, setTotalPages] = useState(1);
  const [currentPage, setCurrentPage] = useState(0);
  const [PythonBeginner, setPythonBeginner] = useState([]);
  ...
  const fetchPaginatedLessons = async (page = 0) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setPythonBeginner(beginnerLessons);
  ...
  const handleSearch = (e) => {
    ...
    setSearchTerm(term);
    const filtered = PythonBeginner.filter(
      (lesson) =>
  ...
  if (loading || statusLoading) return <p className="text-center mt-10">Loading JavaScript lessons and user status...</p>;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <h1 className="text-3xl font-bold text-gray-900">JavaScript Beginner Lessons</h1>
