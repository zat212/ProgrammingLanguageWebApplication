...
export default function JavaSaveLesson() {
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">My Saved Java Lessons</h1>
      </div>

      {savedLessons.filter(item => item.lesson?.subject?.subjectName === "Java").length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {savedLessons
            .filter(savedItem => savedItem.lesson?.subject?.subjectName === "Java")
            .map((savedItem, index) => {
