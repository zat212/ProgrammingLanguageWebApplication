...
const PREREQUISITE_SCORE = 10;

export default function PythonInter() {
  const pageSize = 7;
  ...
  const loadUserScore = () => {
    try {
      ...
      if (storedMarksString) {
        ...
        const score = subjectMarks.Python || 0;
        setJavaScore(score);
  ...
  const fetchPaginatedLessons = async (page = 0) => {
    try {
      const response = await LessonService.getPythonLessons(page, pageSize);
      if (response.status === 200) {
  ...
  if (loading || statusLoading)
    return (
      <p className="text-center mt-10">
        Loading Python Intermediate lessons...
      </p>
    );
