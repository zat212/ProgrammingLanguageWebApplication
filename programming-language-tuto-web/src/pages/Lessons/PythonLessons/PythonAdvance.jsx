...
const PREREQUISITE_SCORE = 30;

export default function PythonAdvance() {
  const pageSize = 7;
  ...
  const loadUserScore = () => {
    try {
      ...
      if (storedMarksString) {
        ...
        if (!authorized) {
          toast.warn(
            `Unlock Advance by scoring at least ${PREREQUISITE_SCORE} in Beginner Java lessons.`
          );
        }
      } else {
  ...
  if (loading || statusLoading)
    return (
      <p className="text-center mt-10">
        Loading Python Advance lessons...
      </p>
    );
